...
class truncMVN(object):
    ...
    """
    def __init__(self, means=None,cov=None,low=None,high=None,low_int_limit=np.array([-10,10]),calc_norm=False):                    
        self.low_int_limit = low_int_limit
        self.low = low
        self.high = high
        
        if calc_norm:
            self.norm = self.normalize(means,cov,high,low)
            
    
    def normalize(self,means=None,cov=None,low=None,high=None):
        """
        ...
        cdf_4,i = mvn.mvnun(self.low_int_limit,high,means,cov)
        
        #calculate CDFs of outside regions
        cdf_3,i = mvn.mvnun(self.low_int_limit,np.array([high[0],low[1]]),means,cov)
        cdf_2,i = mvn.mvnun(self.low_int_limit,np.array([low[0],high[1]]),means,cov)
        
        #calculate CDF of lower-left corner region
        cdf_1,i = mvn.mvnun(self.low_int_limit,low,means,cov)
    ...
    def logpdf(self,data,means=np.array([]),cov=np.array([]),re_norm=True):
        """
        Calculate the logpdf for a truncated MVN
        
        data = 2 x n array containing data to calc PDFs for.
        means: 2 x 1 array value for MVN means
        cov:  2x2 covariance matrix
        
        re_norm: if true, re-calculate normalization of the 
        """
        lpdf = stats.multivariate_normal.logpdf(data,mean=means,cov=cov)
        
        if re_norm:
            norm = self.normalize(means,cov,self.low,self.high)
        else:
            norm = self.norm
    ...
    def pdf(self,data,means=np.array([]),cov=np.array([]),re_norm=True):
        """
        ...
        cov:  2x2 covariance matrix
        """
        pdf = stats.multivariate_normal.logpdf(data,mean=means,cov=cov)
